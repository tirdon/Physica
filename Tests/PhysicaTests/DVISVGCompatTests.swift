import Testing
@testable import PhysicaMath
@testable import PhysicaAlgebra
@testable import PhysicaGeometry
@testable import PhysicaTypesetting
@testable import PhysicaKernel
@testable import PhysicaPlotting
@testable import PhysicaStory
@testable import PhysicaPhysics
@testable import PhysicaEquationGame

// Genuine `dvisvgm --no-fonts --bbox=min` output for `$x=1$` at 10pt (single
// quotes, pt coordinates, no scale(1,-1) group) — proves MathSVG parses the
// dvisvgm dialect once `unitsPerEm` is the document point size, so baked SVG
// assets flow through the same parser as MathJax output.
private let dvisvgmXEquals1 = #"""
<svg version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='23.958684pt' height='6.420368pt' viewBox='76.712329 56.344264 23.958684 6.420368'>
<defs>
<path id='g1-49' d='M2.929016-6.37609C2.929016-6.615193 2.929016-6.635118 2.699875-6.635118C2.082192-5.997509 1.205479-5.997509 .886675-5.997509V-5.688667C1.085928-5.688667 1.673724-5.688667 2.191781-5.947696V-.787049C2.191781-.428394 2.161893-.308842 1.265255-.308842H.946451V0C1.295143-.029888 2.161893-.029888 2.560399-.029888S3.825654-.029888 4.174346 0V-.308842H3.855542C2.958904-.308842 2.929016-.418431 2.929016-.787049V-6.37609Z'/>
<path id='g1-61' d='M6.844334-3.257783C6.993773-3.257783 7.183064-3.257783 7.183064-3.457036S6.993773-3.656289 6.854296-3.656289H.886675C.747198-3.656289 .557908-3.656289 .557908-3.457036S.747198-3.257783 .896638-3.257783H6.844334ZM6.854296-1.325031C6.993773-1.325031 7.183064-1.325031 7.183064-1.524284S6.993773-1.723537 6.844334-1.723537H.896638C.747198-1.723537 .557908-1.723537 .557908-1.524284S.747198-1.325031 .886675-1.325031H6.854296Z'/>
<path id='g0-120' d='M3.327522-3.008717C3.387298-3.267746 3.616438-4.184309 4.313823-4.184309C4.363636-4.184309 4.60274-4.184309 4.811955-4.054795C4.533001-4.004981 4.333748-3.755915 4.333748-3.516812C4.333748-3.35741 4.443337-3.16812 4.712329-3.16812C4.931507-3.16812 5.250311-3.347447 5.250311-3.745953C5.250311-4.26401 4.662516-4.403487 4.323786-4.403487C3.745953-4.403487 3.39726-3.875467 3.277709-3.646326C3.028643-4.303861 2.49066-4.403487 2.201743-4.403487C1.165629-4.403487 .597758-3.118306 .597758-2.86924C.597758-2.769614 .697385-2.769614 .71731-2.769614C.797011-2.769614 .826899-2.789539 .846824-2.879203C1.185554-3.935243 1.843088-4.184309 2.181818-4.184309C2.371108-4.184309 2.719801-4.094645 2.719801-3.516812C2.719801-3.20797 2.550436-2.540473 2.181818-1.145704C2.022416-.52802 1.673724-.109589 1.235367-.109589C1.175592-.109589 .946451-.109589 .737235-.239103C.986301-.288917 1.205479-.498132 1.205479-.777086C1.205479-1.046077 .986301-1.125778 .836862-1.125778C.537983-1.125778 .288917-.86675 .288917-.547945C.288917-.089664 .787049 .109589 1.225405 .109589C1.882939 .109589 2.241594-.587796 2.271482-.647572C2.391034-.278954 2.749689 .109589 3.347447 .109589C4.373599 .109589 4.941469-1.175592 4.941469-1.424658C4.941469-1.524284 4.851806-1.524284 4.821918-1.524284C4.732254-1.524284 4.712329-1.484433 4.692403-1.414695C4.363636-.348692 3.686177-.109589 3.367372-.109589C2.978829-.109589 2.819427-.428394 2.819427-.767123C2.819427-.986301 2.879203-1.205479 2.988792-1.643836L3.327522-3.008717Z'/>
</defs>
<g id='page1'>
<use x='76.712329' y='62.764633' xlink:href='#g0-120'/>
<use x='85.173593' y='62.764633' xlink:href='#g1-61'/>
<use x='95.689673' y='62.764633' xlink:href='#g1-49'/>
</g>
</svg>
"""#

@Suite @MainActor
struct DVISVGCompatTests {
    /// dvisvgm units are pt; at 10pt design size 1 em = 10 pt.
    let unitsPerEm: Real = 10

    @Test func parsesDvisvgmDialectAtPointScale() throws {
        let glyphs = try MathSVG.glyphs(fromSVG: dvisvgmXEquals1, unitsPerEm: unitsPerEm)
        #expect(glyphs.count == 3)  // x, =, 1 in document order
    }

    @Test func glyphsLandInEmRange() throws {
        let glyphs = try MathSVG.glyphs(fromSVG: dvisvgmXEquals1, unitsPerEm: unitsPerEm)
        var union = Bounds.empty
        for glyph in glyphs { union = union.union(glyph.path.bounds) }

        // The line is ~24 pt wide → ~2.4 em, and bounds-centered on the origin.
        let width = union.max.x - union.min.x
        #expect(width > 2.2 && width < 2.5)
        #expect(abs(union.min.x + union.max.x) < 0.05)  // centered

        // Every glyph is under one em tall and the digit '1' is a real chunk of it.
        for glyph in glyphs {
            let height = glyph.path.bounds.max.y - glyph.path.bounds.min.y
            #expect(height < 1.0)
        }
        let oneHeight = glyphs[2].path.bounds.max.y - glyphs[2].path.bounds.min.y
        #expect(oneHeight > 0.5)  // cmr '1' cap-height ≈ 0.64 em
    }

    @Test func defaultScaleIsUnchangedForMathJax() throws {
        // The MathJax default (1000 u/em) must keep working untouched.
        let markup = ##"<svg viewBox="0 0 1 1"><defs><path id="a" d="M0 0H100V-100H0Z"/></defs>"##
            + ##"<g transform="scale(1,-1)"><use xlink:href="#a"/></g></svg>"##
        let glyphs = try MathSVG.glyphs(fromSVG: markup)
        #expect(glyphs.count == 1)
        let box = glyphs[0].path.bounds
        #expect(abs((box.max.x - box.min.x) - 0.1) < 1e-6)  // 100/1000 em
    }
}
