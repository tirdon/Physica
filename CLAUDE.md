...
- **`Sources/PhysicsEngine/`** — thin wasm executable: entry point, `App.swift` (load font/MathJax → build demo scene → `WebRuntime.run`), and `Demos/PendulumDemo.swift`. Files using `JSPromise.value()` must `import JavaScriptEventLoop` explicitly — the async helpers live in that module, and Swift's lenient member-import visibility used to mask it.
...
`swift test` covers everything in the core. For anything touching `Sources/Physica/WASM/` (Render/ or Web/), rebuild the wasm bundle and check visually: headless Chrome renders WebGPU on macOS via puppeteer-core with `--headless=new --use-angle=metal --enable-unsafe-webgpu` against the running bun server (screenshot the canvas at chosen timeline times). `bun scripts/smoke.mjs` is the GPU-free sanity check that the bundle still boots.
