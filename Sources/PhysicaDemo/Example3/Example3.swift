// Example3 — a Medium-style scientific article on the `Document` facade: the
// bare `Document("…") { … }` statement builds the value AND auto-mounts it
// into the page (outline logged first, MathJax injected when absent,
// ArticleDOM walked, typeset). No WebGPU scene — except the embedded
// `presentation {}` deck, which boots its own story canvas.
//
// Build its bundle:
...
struct Example3 {
	static func main() {
		...
		#else
		// Host: the neutral value model builds and its outline prints — proof
		// the article type-checks off-wasm.
		for line in ArticleOutline.lines(
			for: makeDocument(), banner: "Example3 — article outline"
		) {
			print(line)
		}
		#endif
	...
	/// The article, authored on the declarative Document DSL.
	static func makeDocument() -> Document {
		Document("Physics · Rigid bodies") {
			Title(
