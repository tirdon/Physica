...
public struct SwipeRecognizer: Sendable {
    ...
    public var threshold: Real

    private var start: Position?
    private var latest: Position = .zero

    public init(threshold: Real = 1.0) {
        self.threshold = threshold
    }

    /// True between `begin()` and the next `end()`/`cancel()`.
    public var isTracking: Bool { start != nil }

    public mutating func begin(at point: Position) {
        start = point
        latest = point
    }

    public mutating func move(to point: Position) {
    ...
    public mutating func end() -> SwipeDirection? {
