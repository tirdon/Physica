...
public final class Arrow: PathEntity {
    ...
    private func rebuild() {
        ...
        let halfHead = normal * (headWidth / 2)
        var result = Path.line(from: shaftStart, to: shaftEnd)
        let endHead: [SIMD2<Real>] = [b, shaftEnd + halfHead, shaftEnd - halfHead]
        result = result.appending(.polygon(points: endHead))
        if doubleHeaded {
            let startHead: [SIMD2<Real>] = [a, shaftStart + halfHead, shaftStart - halfHead]
            result = result.appending(.polygon(points: startHead))
        }
        path = result
    }
}
