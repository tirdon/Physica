// GeometryUploader — turns a SceneSnapshot into flat GPU-ready arrays + a draw
// list. Per-frame rebuild: one flat-vertex stream, one mesh stream, one uniform
// block. This is the macOS sibling of `Sources/WASM/Renderer/GeometryUploader`,
// ported with two differences:
//   • it also emits `.image` commands (emoji clusters + `Image` entities) — the
//     browser draws those in a DOM layer, but native has no DOM, so they render
//     as textured quads *after everything* (always above painter's order);
//   • `Real` is `Double` on macOS, so vertex/uniform floats are converted to
//     `Float32` explicitly (the wasm version relied on `Real == Float`; the
//     shared kernel is never touched).
...
/// Where a textured quad's bitmap comes from — a URL/data:/path (`Sprite` and
/// `Image` entities) or an emoji cluster the renderer rasterizes with CoreText.
...
struct DrawCommand {
    enum Kind {
        ...
        case meshOutline
        /// Textured quad (`Sprite`) in 2D painter's order.
        case sprite
        /// Textured quad (emoji glyph box / `Image` entity) drawn after
        /// everything — the native replacement for the browser's DOM layer.
        case image
    }

    var kind: Kind
    /// Vertex range in the flat stream (pathStencil/pathCover/stroke/sprite/image).
    ...
    /// Bitmap source for `.sprite`/`.image` commands.
    var texture: TextureSource? = nil
}
...
enum GeometryUploader {
    static func pack(_ snapshot: borrowing SceneSnapshot) -> FramePacket {
        ...
        // Images last: emoji glyph boxes and `Image` entities are always drawn
        // above the canvas (the browser floats them in a DOM layer; here they
        // are textured quads appended after every path/mesh/sprite).
        for primitive in snapshot.primitives {
            if case .image(let image) = primitive {
                appendImage(image, into: &packet)
            }
        }
        return packet
    }
    ...
    private static func appendUniformSlot(
        ...
        packet.uniforms.reserveCapacity(packet.uniforms.count + FramePacket.uniformSlotFloats)
        packet.uniforms.append(contentsOf: model.floatArray)
        packet.uniforms.append(color.r)
        packet.uniforms.append(color.g)
        packet.uniforms.append(color.b)
        packet.uniforms.append(color.a)
        packet.uniforms.append(contentsOf: params)
        let pad = FramePacket.uniformSlotFloats - 24
        // repeatElement is a lazy Collection — no transient pad array per draw.
        packet.uniforms.append(contentsOf: repeatElement(0 as Float32, count: pad))
        return offset
    }
    ...
    private static func appendMesh(_ mesh: borrowing MeshDraw, into packet: inout FramePacket) {
        ...
        for index in 0..<mesh.positions.count {
            ...
            // Direct scalar appends — no 6-element transient array per vertex.
            packet.meshVertices.append(Float32(p.x))
            packet.meshVertices.append(Float32(p.y))
            packet.meshVertices.append(Float32(p.z))
            packet.meshVertices.append(Float32(n.x))
            packet.meshVertices.append(Float32(n.y))
            packet.meshVertices.append(Float32(n.z))
        }
    ...
    // MARK: Textured quads (sprites in painter's order; images always above)

    private static func appendSprite(_ sprite: borrowing SpritePrimitive, into packet: inout FramePacket) {
        appendTexturedQuad(
            center: sprite.center, size: sprite.size, rotation: sprite.rotation,
            opacity: sprite.opacity, source: .url(sprite.url), kind: .sprite, into: &packet
        )
    }

    private static func appendImage(_ image: borrowing ImagePrimitive, into packet: inout FramePacket) {
        let source: TextureSource = image.url.map { .url($0) } ?? .emoji(image.text)
        appendTexturedQuad(
            center: image.center, size: image.size, rotation: image.rotation,
            opacity: image.opacity, source: source, kind: .image, into: &packet
        )
    }
    ...
    private static func appendTexturedQuad(
        center: Position, size: SIMD2<Real>, rotation: Real, opacity: Real,
        source: TextureSource, kind: DrawCommand.Kind, into packet: inout FramePacket
    ) {
        guard size.x > 1e-6, size.y > 1e-6, opacity > 0.001 else { return }
        let slot = appendUniformSlot(
            model: .identity,
            color: Color.white.with(opacity: Float(opacity)),
            params: [Float32(size.x / size.y), 0, 0, 0],
            into: &packet
        )
        let halfW = size.x / 2
        let halfH = size.y / 2
        ...
        func corner(_ x: Real, _ y: Real) -> Position {
            Position(center.x + x * cosR - y * sinR, center.y + x * sinR + y * cosR, center.z)
        }
        ...
        packet.commands.append(DrawCommand(
            kind: kind, vertexStart: start, vertexCount: 6, uniformOffset: slot, texture: source
        ))
    }
    ...
    private static func appendStroke(_ path: borrowing PathPrimitive, color: Color, into packet: inout FramePacket) {
        ...
        // plus fractional ends on the boundary segments.
        ...
        if path.style.neon {
            // Neon tube: stacked translucent halos + a hot whitened core, all
            // over the same trim window.
