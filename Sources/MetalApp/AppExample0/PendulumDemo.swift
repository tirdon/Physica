...
//
// Copied from Sources/PhysicaDemo/Example1/PendulumDemo.swift — targets can't
// share sources, so the native demo carries its own copy (import PhysicaApp
// instead of Physica; #if os(macOS) instead of #if os(WASI)). Native has no
// MathJax, so it is always built with a nil formula (the demo tolerates it).
...
enum PendulumDemo {
    static func build(_ scene: Scene, font: Font?, formula: TextEntity? = nil) {
        ...
        // No MathJax (native) → formula is nil and both plays enqueue nothing —
        // optional-tolerant `play` replaces the old `if let` guard.
